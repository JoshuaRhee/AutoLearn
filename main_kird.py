...
while wait_for_lecture:
    ...
    while not flag_end:
        try: # Show control box
            ...
            tot_time = round(float(driver.find_element(By.TAG_NAME,"video").get_attribute('duration')))
            
            for t in range(tot_time):
                print(f'영상 시청 중 ... ({t+1} / {str(tot_time)})')
                time.sleep(0.01)
            ...
            try:
                ...
                try:
                    ...
                    while True:
                        ...
                        break
                    driver.switch_to.parent_frame()
                except: # Maybe the last page
                    print('마지막 페이지를 발견했습니다.')
                    flag_end = True
